...
/*   Updated: 2019/06/03 13:28:53 by vrabaib          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*julia_driver(void *data)
{
	...
	while (++frac->pfrac.pfrac_x <= WIDTH)
	{
		...
		while (++frac->pfrac.pfrac_y <= frac->pfrac.pfrac_ymax)
		{
			...
			frac->c.x_map = mapping(frac->c.x0, WIDTH, \
			-2.5, 1) / frac->prop.zoom;
			frac->c.y_map = mapping(frac->c.y0, HEIGHT, \
			-1, 1) / frac->prop.zoom;
			frac->c.x = 0;
			frac->c.y = 0;
			julia_check(frac);
...
void	julia_check(t_frac *frac)
{
	...
	{
		xtmp = SQR(frac->c.x_map) - SQR(frac->c.y_map) + \
		mapping(frac->mouse.mouse_x, WIDTH, -1, 1);
		frac->c.y_map = 2 * frac->c.x_map * frac->c.y_map + \
		mapping(frac->mouse.mouse_x, HEIGHT, -1, 1);
		frac->c.x_map = xtmp;
		iteration++;
	}
...
void	julia_plot(double iteration, t_frac *frac)
{
	...
	{
		log_zn = log(frac->c.x_map * frac->c.x_map + \
		frac->c.y_map * frac->c.y_map) / 2;
...
void	julia_thread(t_frac *frac)
{
	...
	while (i < THREADS)
	{
		...
		pthread_create(&trds[i], NULL, julia_driver, &fracs[i]);
		i += 1;
	}
	while (i--)
