...
/*   Updated: 2019/06/03 15:59:50 by vrabaib          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fractal_instructions(t_frac *frac)
{
	...
	max_iteration = ft_itoa(frac->prop.max_iteration);
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 60, \
	HEIGHT / 80, WHITE, "MOVE : WASD");
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 60, \
	HEIGHT / 20, WHITE, "SCROLL +/- : MOUSE WHEEL");
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 60, \
	HEIGHT / 11, WHITE, "ITERATION +/- : E/X");
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 60, \
	HEIGHT / 7.5, WHITE, "COLOR : C");
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 60, \
	HEIGHT / 5.75, WHITE, "RESET : R");
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 60, \
	HEIGHT / 4.65, WHITE, "CLOSE : ESC");
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 60, \
	HEIGHT / 1.05, WHITE, "MAX ITERATIONS : ");
	mlx_string_put(frac->image.img, frac->display.win, WIDTH / 5, \
	HEIGHT / 1.05, WHITE, max_iteration);
	free(max_iteration);
}
...
{
	if (x < WIDTH && y < HEIGHT)
		ft_memcpy(frac->image.data + 4 * WIDTH * y + \
		4 * x, &color, sizeof(int));
}
